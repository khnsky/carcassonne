...
static int __completionToStatus(bool isCompleted) {
    if (isCompleted) { return 1; } else { return -1; }
}
...
int score(sized_board* board) {
        ...
        for (size_t j = 0; j < columns; j++) {
            ...
            if (!tile_isEmpty(t)) {
                ...
                if (tile_hasCastle(t)) {
                    ...
                    if (tile_hasShield(t)) { castleScore++; }
                    ...
                    // SPECIAL CASE
                    // when castleSegments == 2 - there can be 2 cities:
                    // each segment then should be calculated individually
                        ...
                        bool completed = true;
                        for (size_t k = 0; k < castleSegments; k++) {
                            completed &= castleCompleted(
                                tiles, rows, columns, i, j, sides[k]
                                );
                            if (!completed) { break; }
                        }
                        if (completed) { castleScore += 2; }
                        else { castleScore += 1; }
                    }
                    score += castleScore;
                    CS += castleScore;
                ...
                if (tile_hasRoad(t)) {
                    ...
                    if(roadSegments == 2) {
                        roadScore += roadScoreForTwo(
                            tiles, rows, columns, i, j, sides
                            );
                    } else {
    ...
    // clear all incompleted side statuses
    // in further executions this status may be updated, so it can't be constant
    for (size_t i = 0; i < rows; i++) {
        for (size_t j = 0; j < columns; j++) {
            tile* t = tiles[i][j];
            if (!tile_isEmpty(t)) {
                for(int k = 0; k < 4; k++) { tile_setSideCompletion(t, k, 0); }
            }
        }
    }
    return score;
...
bool castleCompleted(board_t board, size_t rows, size_t columns,
    ...
    switch (status) {
    case -1: return false;  // side was visited and assigned as an uncompleted
    case  1: return true;   // side was visited and assigned as completed
    case  0: break;         // side is not visited yet
    }
...
bool tile_castleCompleted(board_t board, size_t rows, size_t columns,
    ...
    // adding a tile to a list of visited tiles
    // (later every side of every tile from list will have
    // the indicator of true/false completion
    ...
    if (numOfCastles == 1) { return true; }

    // 2 disjoint city segments 
    if (numOfCastles == 2 && tile_getCenter(t) != CITY) { return true; }
    ...
    for (size_t k = 0; k < numOfCastles; k++) {
        ...
        switch (sides[k]) {
        case NORTH:
            // if open side is board's border => castle is not completed
            if (i <= 0) { compl = false; }
            else {
                in = i - 1;
                jn = j;
        ...
        //  2) We've came from this tile
        //  (can be recognised when dir(previous direction of movements)
        //  is opposite to sides[k] (new direction of movement)
        //  3) We are heading to the initial tile's unvisited side
        if (compl                                                           &&
            !List_hasPoint(stack, in, jn, direction_getOpposite(sides[k]))  &&
            !direction_areOpposite(dir, sides[k])) {
        ...
        // --if any side gave false completion
        // the overall completion status changes immediately
        // --if side gave true completion - the overall completion status remains true
        isCompl &= compl;

        // if already detected open side
        // => city is not completed
...
bool tile_roadCompleted(board_t board, size_t rows, size_t columns,
    ...
    // if the status of the side is already stated -> no use of further investigation
    switch (tile_getSideCompletion(t, direction_getOpposite(dir))) {
    case COMPL_NOT_COMPLETED: return false;
    case COMPL_COMPLETED:     return true;
    }

    // adding a tile to a list of visited tiles
    // (later every side of every tile from list
    // will have the indicator of true / false completion
    List_addLast(stack, point_new(i, j, direction_getOpposite(dir)));

    if (tile_hasCrossroads(t)   ||
        tile_hasTemple(t)       ||
    ...
    if (tile_getSideElement(t, NORTH) == ROAD           &&
        ...
        in = i - 1;
        dir = NORTH;
    } else if (tile_getSideElement(t, EAST) == ROAD     &&
               j != columns - 1                         &&
               dir != WEST) {
        jn = j + 1;
        dir = EAST;
    } else if (tile_getSideElement(t, SOUTH) == ROAD    &&
               i != rows - 1                            &&
               dir != NORTH) {
        in = i + 1;
        dir = SOUTH;
    } else if (tile_getSideElement(t, WEST) == ROAD     &&
               j != 0                                   &&
               dir != EAST) {
        jn = j - 1;
        dir = WEST;
...
int roadScoreForTwo(board_t board, size_t rows, size_t columns,
    ...
    switch(
        side_isCompleted(
            tile_getSide(t, sides[0])
            ) * side_isCompleted(tile_getSide(t, sides[1]))
        ) {
        case  1: return 2;
        case -1: return 1;
    }
...
int tile_numOfNeighbours(board_t board, size_t rows, size_t columns,
    ...
    if (i > 0) {
        if (!tile_isEmpty(board[i - 1][j]))                        { count++; }
        if (j > 0           && !tile_isEmpty(board[i - 1][j - 1])) { count++; }
        if (j < columns - 1 && !tile_isEmpty(board[i - 1][j + 1])) { count++; }
    }
    ...
    // central column
    if (j > 0           && !tile_isEmpty(board[i][j - 1])) { count++; }
    if (j < columns - 1 && !tile_isEmpty(board[i][j + 1])) { count++; }
    return count;
}
